...
export default {
  ...
  mounted() {
    var _this = this;
    EventBus.$on("update-area", function (area) {
      _this.area.id = area.id;
      _this.area.name = area.area_name;
      _this.area.english_name = area.area_english_name;
      _this.area.status = area.status;
  ...
  methods: {
    store() {
      ...
      axios
        .put(base_url + "admin/area/" + this.area.id, this.area)
        .then((response) => {
